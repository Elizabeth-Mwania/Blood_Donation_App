...
def load_data():
    try:
        # Updated to load only two CSV files
        donor_candidates_birth = pd.read_csv('data/data_2019_cleaned.csv')
        donors = pd.read_csv('data/Donnors_2019.csv')
        
        # Rename columns to match new variable names
        candidates_columns = {
        ...
        donor_candidates_birth.rename(columns=candidates_columns, inplace=True)
        donors.rename(columns=donors_columns, inplace=True)
        
        # Convert date columns
        date_columns = [col for col in donor_candidates_birth.columns if 'date' in col.lower()]
        for col in date_columns:
            donor_candidates_birth[col] = pd.to_datetime(donor_candidates_birth[col], errors='coerce')
        
        # Add eligibility flag
        if 'eligibility' in donor_candidates_birth.columns:
            donor_candidates_birth['is_eligible'] = donor_candidates_birth['eligibility'].apply(
...
# Train and load ML model
@st.cache_resource
def load_models():
    try:
        # Simulate training a model based on earlier candidate data
        np.random.seed(42)
        df = pd.DataFrame({
            'age': np.random.randint(18, 65, 405),
            'hemoglobin_level': np.random.normal(14, 1.5, 405),
            'weight': np.random.normal(70, 15, 405),
            'gender': np.random.choice([0, 1], 405),  # 0: Homme, 1: Femme
            'last_donation_days': np.random.choice([9999] + list(range(0, 180)), 405),
            'is_eligible': [1]*365 + [0]*40  # Simulated 10% ineligible
        })
...
if page == "Overview":
    ...
    if donor_candidates_birth is not None and donors is not None:
        with metrics_row[0]:
            ...
            st.markdown(f'<div class="metric-container"><h3>Total Candidates</h3><p class="highlight" style="font-size: 2rem;">{total_candidates}</p></div>', unsafe_allow_html=True)
        with metrics_row[1]:
            total_donors = len(donors)
            conversion_rate = round((total_donors / total_candidates) * 100, 1)
            st.markdown(f'<div class="metric-container"><h3>Total Donors</h3><p class="highlight" style="font-size: 2rem;">{total_donors}</p><p>Conversion Rate: {conversion_rate}%</p></div>', unsafe_allow_html=True)
        with metrics_row[2]:
            if 'gender' in donors.columns:
                gender_counts = donors['gender'].value_counts()
                male_pct = round((gender_counts.get('Homme', 0) / total_donors) * 100, 1)
                female_pct = round((gender_counts.get('Femme', 0) / total_donors) * 100, 1)
                st.markdown(f'<div class="metric-container"><h3>Gender Distribution</h3><p>Male: <span class="highlight">{male_pct}%</span></p><p>Female: <span class="highlight">{female_pct}%</span></p></div>', unsafe_allow_html=True)
        with metrics_row[3]:
            if 'donation_type' in donors.columns:
                ...
                st.markdown(f'<div class="metric-container"><h3>Donation Types</h3><p>Voluntary: <span class="highlight">{voluntary_pct}%</span></p><p>Family: <span class="highlight">{family_pct}%</span></p></div>', unsafe_allow_html=True)
    ...
    if donors is not None:
        with charts_row[0]:
            ...
            if 'blood_group' in donors.columns:
                ...
                fig = px.pie(values=blood_group_counts.values, names=blood_group_counts.index, color_discrete_sequence=px.colors.sequential.Reds, hole=0.4, title="Distribution of Blood Groups")
    ...
    if donor_candidates_birth is not None:
        filtered_df = apply_filters(donor_candidates_birth)
        charts_row = st.columns(2)
        with charts_row[0]:
            st.markdown("<div class='chart-container'>", unsafe_allow_html=True)
            st.subheader("Eligibility by Health Condition")
            #rejection_counts = [
            #    filtered_df['ineligible_antibiotics'].value_counts().get('Yes', 0),
            #    filtered_df['ineligible_low_hemoglobin'].value_counts().get('Yes', 0),
                #filtered_df['ineligible_recent_donation'].value_counts().get('Yes', 0),
            #    filtered_df['ineligible_recent_sti'].value_counts().get('Yes', 0)
            #]
        ...
        with metrics_row[0]:
            ...
            systolic = np.random.normal(120, 15, 200)  # Placeholder
        ...
        with metrics_row[2]:
            ...
            reasons = ['Low Hb', 'Surgery']
            #reasons = ['Low Hb', 'Pregnancy', 'Surgery']
            counts = [
                filtered_df['ineligible_low_hemoglobin'].value_counts().get('Yes', 0),
                #filtered_df['ineligible_recent_donation'].value_counts().get('Yes', 0),
                #filtered_df['female_ineligible_pregnant'].value_counts().get('Yes', 0),
                filtered_df['total_ineligible_surgery'].value_counts().get('Yes', 0)
            ]
    ...
    if donor_candidates_birth is not None and 'other_reasons' in donor_candidates_birth.columns:
        feedback = donor_candidates_birth['other_reasons'].dropna().tolist()
    else:
        feedback = [
    ...
    with st.form("new_data_form"):
        ...
        with col1:
            ...
            "Birth Date",
            min_value=date(1900, 1, 1),  # Allows dates from year 1900
            max_value=date.today(),       # Up to current date
            value=date.today()            # Default value (optional)
            )
        ...
        if submit:
            new_candidate = {
                'form_fill_date': datetime.now(),'firstname': firstname, 'name': name, 'birth_date': birthdate, 'age': age, 'gender': gender, 'weight': weight,
                'hemoglobin_level': hemoglobin, 'residence_district': district, 'has_donated_before': 'Yes' if donation_freq > 0 else 'No',
                ...
                'donation_frequency': donation_freq, 'campaign_channel': campaign_channel
            }
    ...
    if len(st.session_state.new_candidates) > 0:
        ...
        st.download_button(
            label="Download Candidates Data",
            data=candidates_csv,
            file_name="blood_donation_candidates.csv"+str(datetime.now()),
            mime="text/csv"
        )
    if len(st.session_state.new_donors) > 0:
        donors_csv = st.session_state.new_donors.to_csv(index=False)
        st.download_button(
            label="Download Donors Data",
            data=donors_csv,
            file_name="blood_donation_donors.csv"+str(datetime.now()),
            mime="text/csv"
        )
